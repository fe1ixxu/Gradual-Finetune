...
def get_domain_index(args):
    global START_INDEX, END_INDEX
    if args.domain == "restaurant":
        START_INDEX = 18
        END_INDEX = 25
    elif args.domain == "hotel":
        START_INDEX = 8
        END_INDEX = 18
    elif args.domain == "taxi":
        START_INDEX = 25
        END_INDEX = 29
    elif args.domain == "train":
        START_INDEX = 29
        END_INDEX = 35
    elif args.domain == "attraction":
        START_INDEX = 0
        END_INDEX = 3
    else:
        raise ValueError("No such {} domain!, only restaurant, hotel, taxi, train, attraction approved!".format(args.domain))
...
class Processor(DataProcessor):
    ...
    def __init__(self, config):
        ...
        elif "multiwoz" in config.data_dir: #config.data_dir == "data/multiwoz":
...
def main():
    ...
    parser.add_argument("--data_augmentation",
                        default=None,
                        help="use out-of-domain data during training")
    parser.add_argument("--fine_tune",
                        default=None,
                        help="Fine-tuning a pretrained model")
    ...
    # get the index of the domain we want to test:
    get_domain_index(args)
    ...
    if args.do_train:
        train_examples = processor.get_train_examples(args.data_dir, args.data_augmentation, accumulation=accumulation)
    ...
    # Fine-tuning
    ###############################################################################
    # Load a trained model that you need to fine-tune
    if args.fine_tune:
        output_model_file = os.path.join(args.fine_tune, "pytorch_model.bin")
        model = BeliefTracker(args, num_labels, device)
        ...
        if n_gpu == 1:
            ...
            print("Fine-tuning using only one device!")
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for input_ids, input_len, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            accuracies = eval_all_accs(pred_slot, label_ids, accuracies, device)
        ...
        with open(os.path.join(args.output_dir, "%s.txt" % out_file_name), 'w') as f:
            ...
            print("Test Acc:", accuracies['slot_rest'] / accuracies['num_slot_rest'].item(),
                (accuracies['joint_rest']/accuracies['num_turn']).item())

def eval_all_accs(pred_slot, labels, accuracies, device):
    ...
    def _eval_acc(_pred_slot, _labels):
        ...
        # joint accuracy
        # joint_acc = sum(torch.sum(accuracy, 1) / float(slot_dim)).float()
        joint_acc = torch.sum(accuracy, 1) / float(slot_dim)
        zeros = torch.zeros(joint_acc.shape).to(device)
        joint_acc = torch.sum(torch.where(joint_acc < 1., zeros , joint_acc))
    ...
    # target domain
    joint_acc, slot_acc, num_turn, num_data = _eval_acc(pred_slot[:,:,START_INDEX:END_INDEX], labels[:,:,START_INDEX:END_INDEX])
